...
# Creating our matrixes. It's necessary to make 1000 copies of a zeroed line
zeroed_line = [0]*1000
light_matrix = [zeroed_line[:] for i in range(1000)]
bright_matrix = [zeroed_line[:] for i in range(1000)]

# Processing the instructions
for i in instructions:
  # Removing the verbosity of the instructions
  i = i.replace("turn ","").replace(",", " ").replace(" through", "")
  oot, x1, y1, x2, y2 = i.split(" ")
...
for x in range(0,1000):
  for y in range(0,1000):
    sum_lit += light_matrix[x][y]
    sum_bright += bright_matrix[x][y]
    
print(f"Part One: {sum_lit}")
print(f"Part Two: {sum_bright}")