# Declaring the input as a literal string, to avoid the parsing from Python interpreter
...
sum_code = 0   # "Raw" string
sum_decode = 0  # "Decoded" string
sum_encode = 0 # "Encoded" string

for line in input.splitlines():
  # Raw count of characters
  code_len = len(line)
  
  # Calculating the decoding the string
  decode_len = -2 # discounting the start and the end double quotes (")
  escaping = 0 # kind of flag for escaping control
  for c in line:
    if escaping == 0 and c == '\\':
      escaping = 1
    elif escaping == 1 and c == 'x':
      escaping = 2
    elif escaping > 1:
      escaping -= 1
    else:
  ...
  # Calculating the encoding of the string
  encode_len = code_len + 2 # adding the start and end double quotes (")
  for c in line:
    if c == '\\' or c == '"':
      encode_len += 1 # Adding 1 more character for backslash (\) and double quotes (")
  # print(f"{line}: {decode_len}/{code_len}/{encode_len}")
  
  # Accumulating the sums
  sum_code += code_len
  sum_decode += decode_len
  sum_encode += encode_len
  
print(f"Part One: {sum_code-sum_decode}")

print(f"Part Two: {sum_encode-sum_code}")
